...
(`libracargo_web`): un chequeo de credenciales **server-to-server**, protegido
...
En `suitrans` y en cualquier instancia de cliente la ruta no existe. La otra
mitad —el usuario del visitante y el repositorio de códigos— la cablea
`main.py`; son dos decisiones separadas y cada una falla distinto.

Sigue **sin** `incluir_password_reset`: eso necesita SMTP configurado, que
LibraCargo todavía no tiene. Un endpoint de recuperación sin SMTP contesta 503
y confunde.
