...
tal cual, pasándole el `basePath`—, con dos diferencias propias de LibraCargo:
el prefijo va en castellano como el resto de su API, y **todo el router exige
rol admin**.
...
def repositorio(request: Request) -> UserRepository:
    ...
    tabla es una fábrica de sesiones de más, y el de `app.state` es el que usa
    el login.
    """
...
class UsuarioEditado(BaseModel):
    ...
    role: Rol
    active: bool = True
    #: `None` es "dejalo como está"; `""` lo borra. El default **tiene** que ser
    #: `None`: el botón de activar/desactivar de la grilla manda este mismo
    #: cuerpo sin tocar el correo, y con `""` por default desactivar a alguien
    #: le borraba el mail en silencio.
    email: str | None = None
...
def crear(datos: UsuarioNuevo, usuarios: UserRepository = Depends(repositorio)):
    try:
        return usuarios.create(datos.username, datos.name, datos.password, datos.role,
                               email=datos.email)
...
def editar(id_: str, datos: UsuarioEditado,
    ...
    la única salida es entrar a la base. La regla mira el usuario de la sesión,
    no el cuerpo del pedido.
    """
...
def cambiar_clave(id_: str, datos: ClaveNueva, usuarios: UserRepository = Depends(repositorio)):
...
def eliminar(id_: str, usuarios: UserRepository = Depends(repositorio),
             actual: dict = Depends(get_current_user)):
