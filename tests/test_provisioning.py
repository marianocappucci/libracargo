...
def test_la_instancia_de_dev_corre_las_mismas_migraciones_que_el_deploy(script):
    ...
    repo, y hasta el 2026-08-25 ahí no había ningún paso de Alembic: apareció
    con la base en `0009` y el código en `0010`, o sea el defecto del 2026-08-24
    entero, en la instancia donde se trabaja todos los días.
    ...
    es que agregue una segunda cadena en `scripts/` —como tienen Gestiolibra y
    MedLibra— y se olvide del compose: ahí `dev` migraría de menos y el error
    culparía a la revisión equivocada.
    ...
    for comando in declarados:
        ...
        assert pos != -1, (
            ...
            "esquema viejo, que es lo que pasó el 2026-08-25."
        )
...
def test_el_backoffice_puede_listar_los_planes():
    ...
    (medido en LibraClub el 2026-09-15, mismo hueco acá). Se prueba por el
    camino del motor, `planes_info()`, y no sólo la función suelta — lo que
    importa es el contrato que el motor espera.
    """
