...
`POST /auth/verify` es lo que usa el login de `/docs/` de `libracargo_web` para
validar credenciales sin crear sesión. `POST /auth/demo` es el auto-login del
visitante de `demo.libracargo.com.ar`, con código de acceso.
...
Igual que en `test_auth.py`, `TestClient` va con `base_url="https://…"`: la
...
def cliente_con_secreto(entorno):
    ...
    return TestClient(crear_app(_cfg()), base_url="https://testserver")
...
def test_verify_no_crea_sesion(cliente_con_secreto):
    ...
    sesión de la app del cliente en un origen que no es el suyo."""
    r = cliente_con_secreto.post(
        "/auth/verify",
        json={"username": USUARIO, "password": CLAVE},
        headers={"X-Internal-Auth": SECRETO_DOCS},
    ...
    assert r.json() == {"valid": True}
    assert COOKIE not in cliente_con_secreto.cookies
...
def test_verify_falla_cerrado_sin_secreto_configurado(entorno):
    ...
    cliente = TestClient(crear_app(_cfg()), base_url="https://testserver")
...
def test_sin_demo_mode_la_ruta_de_demo_no_atiende(entorno):
    """En una instancia de cliente —`suitrans`— el auto-login **no existe**.

    Se afirma "no es 200" y no un código exacto a propósito: LibraCargo sirve
    la SPA con un catch-all, así que una ruta inexistente puede contestar 405
    (el catch-all matchea por GET y el POST cae en método no permitido) en vez
    de 404. Lo que no puede pasar nunca es que deje entrar.
    """
...
def test_con_un_codigo_emitido_se_entra(demo):
    ...
    assert r.status_code == 200, r.text
    assert COOKIE in cliente.cookies


def test_el_visitante_de_la_demo_no_es_admin(demo):
    """🔴 Un visitante con rol admin llega a Usuarios, a Configuración y al
    backup — o sea, puede vaciar la demo y ver lo que no es la muestra."""
    cliente, app = demo
    emitido = app.state.demo_codigos.crear(etiqueta="suite")
    r = cliente.post("/auth/demo", json={"codigo": emitido["codigo"]})
    assert r.status_code == 200, r.text
