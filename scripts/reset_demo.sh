#!/usr/bin/env bash
# Reset diario de la demo publica de LibraCargo.
#
...
#
# Corre por cron a las 05:00, despues de los resets de las otras seis demos
...
# un cliente --y este producto tiene uno real, `libracargo-suitrans`-- y no hay
# confirmacion que valga a las cinco de la manana.
#
# Copiado del reset de Ventalibra, que acumula las defensas de tres incidentes
# de esta familia. La diferencia propia esta marcada con [LIBRACARGO].
set -euo pipefail

CONTENEDOR="libracargo-demo"
CHECKOUT="/root/libracargo"
URL_PUBLICA="https://demo.libracargo.com.ar"
...
SEED_LOCAL=/tmp/seed-libracargo-demo.py
...
# --- 1. El sidecar --------------------------------------------------------
# [LIBRACARGO] Este producto es PostgreSQL siempre, asi que no hay motor que
...
filas_del_dominio() {
  docker exec "$SIDECAR" sh -c '
    psql -tA -U "$POSTGRES_USER" -d "$POSTGRES_DB" -c "
      SELECT COALESCE((SELECT COUNT(*) FROM terceros), 0)
           + COALESCE((SELECT COUNT(*) FROM ordenes_carga), 0)
           + COALESCE((SELECT COUNT(*) FROM movimientos_caja), 0)"
  ' 2>/dev/null || echo "?"
}
...
docker exec -i "$CONTENEDOR" sh -c "
  python3 /tmp/seed.py \
    --url $URL_PUBLICA \
    --usuario \"\${LIBRACARGO_ADMIN_USERNAME:-admin}\" \
    --password \"\$LIBRACARGO_ADMIN_PASSWORD\"
"
